...
group_countries <- cbind(Month = rownames(group_countries), group_countries)
...
group_countries[] <- sapply(group_countries, as.numeric)
...
group_countries$Case_sum <- rowSums(group_countries[2:191])

group_countries$Millions_of_cases <- group_countries$Case_sum/1000000

group_countries <- group_countries %>%
  select(Month, Millions_of_cases )

#To remove Nov data
group_countries <- group_countries[-c(11), ]
group_countries


total_cases_chart <- ggplot(group_countries, aes(x=Month, y=Millions_of_cases)) + geom_bar(stat='identity', fill='steelblue') + labs(title='Total cases from Jan to Oct 2020') + ylab(label='Million of cases') + scale_x_discrete(limit = c('Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct')) + theme(axis.title.x=element_blank()) 


total_cases_chart

ggsave('total_cases_chart.jpeg')
```
```{r}
